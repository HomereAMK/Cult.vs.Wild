...
plot_nearby_genes <- function(outlier_sites, nearby_genes){
  ggplot(nearby_genes, aes(ymin=pos_start, ymax=pos_end, y=(pos_start+pos_end)/2, x=0)) +
    ...
    geom_label(aes(label=abbr, x=-0.2), size=5, position=position_jitter(1, 0, 1)) +
...
plot_nearby_genes <- function(outlier_sites, nearby_genes){
  ...
  mutate(nearby_genes, abbr=ifelse(nchar(abbr>10), str_extract(abbr, "[^ ]+"), abbr), abbr=ifelse(abbr=="unknown", NA, abbr)) %>%
    ...
    geom_text(aes(label=abbr, x=(pos_start+pos_end)/2, y=strand), size=4, position = position_nudge(y=0.5)) +
...
genome_selection_sites_outliers <- data.frame(lg = c("scaffold1", "scaffod1", "scaffold2", "scaffold2"),
                                              pos_bin = c("(1,1e+06]", "(1,1e+06]","(1e+08,2e+08]", "(1e+08,2e+08]"),
                                              min_pos = c(86302500, 72577500, 104557500, 25822500),
                                              max_pos = c(86317500, 75817500,104572500 , 25837500),
                                              length = c(15000, 15000, 15000 ,15000))
